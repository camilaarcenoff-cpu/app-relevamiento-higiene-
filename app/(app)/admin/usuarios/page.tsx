...
export default async function UsuariosPage() {
  ...
  // Asignaciones de la semana más reciente, con foto
  let asignacionesPorUsuario: Record<
    string,
    { id: string; manzana_id: string; codigo: string; barrio: string | null; foto_url: string | null }[]
  > = {};
        ...
        {(usuarios ?? []).map((u) => {
          ...
          return (
            <div key={u.id} className="bg-white rounded-xl shadow-sm p-4">
              ...
              <form action={actualizarUsuario} className="flex flex-wrap gap-3 items-center">
                ...
                <label className="flex items-center gap-1 text-sm text-gray-600">
                  <input type="checkbox" name="activo" defaultChecked={u.activo ?? true} />
                  Activo
                </label>
                <button type="submit" className="text-turquesa text-sm font-semibold hover:underline">
                  Guardar
                </button>
              </form>
                  ...
                  {manzanas.slice(0, 2).map((m) => (
                    <div key={m.manzana_id} className="flex gap-3 items-start">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <form action={subirFotoManzana} className="mt-1">
                          ...
                          <div className="flex gap-2 items-center">
                            ...
                            >
                              {m.foto_url ? "Actualizar" : "Cargar foto"}
                            </button>
                          </div>
                        </form>
                      </div>
              ...
              {manzanas.length === 0 && (
                <p className="mt-2 text-xs text-gray-400 italic">Sin manzana asignada.</p>
              )}
            </div>
          );
        })}
